...
# Created here only when this is the first thing installed. deploy/install.sh
# gives the directory to the openmmo group so the servers can walk to the
# signing key in it, and re-forcing root:root here would take that back.
