...
/*
 * // Controls text style -NOT USED- // NOTE: Text style is defined by control typedef struct
 * GuiTextStyle { unsigned int size; int charSpacing; int lineSpacing; int alignmentH; int
 * alignmentV; int padding; } GuiTextStyle;
 */
...
int GuiColorBarHue(Rectangle bounds, const char *text, float *hue)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        else if (CheckCollisionPointRec(mousePoint, bounds) || CheckCollisionPointRec(mousePoint, selector))
        {
            ...
            /*
             * if (GUI_KEY_DOWN(KEY_UP)) { hue -= 2.0f; if (hue <= 0.0f) hue = 0.0f; } else if
             * (GUI_KEY_DOWN(KEY_DOWN)) { hue += 2.0f; if (hue >= 360.0f) hue = 360.0f; }
             */
        }
    }
...
static int GuiScrollBar(Rectangle bounds, int value, int minValue, int maxValue)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        else if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            /*
             * if (isVertical) { if (GUI_KEY_DOWN(KEY_DOWN)) value += 5; else if
             * (GUI_KEY_DOWN(KEY_UP)) value -= 5; } else { if (GUI_KEY_DOWN(KEY_RIGHT)) value
             * += 5; else if (GUI_KEY_DOWN(KEY_LEFT)) value -= 5; }
             */
        }
