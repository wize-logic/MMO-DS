/* The MonMMO transport session handshake and its crypto state. */
...
/* The pinned MonMMO server root public key: the uncompressed P-256 point of the
...
typedef struct {
    ...
    u8     signature[MMO_HS_MAX_SIG];     /* DER ECDSA over ephemeral_pub */
    size_t siglen;
 ...
 * root key, runs ECDH, derives the session crypto, and appends the framed (still plaintext)
 * ClientReady to `out`.
 */
