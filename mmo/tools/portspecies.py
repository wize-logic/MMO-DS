...
def render_header(rows, machines=None):
    ...
    out = [
        "/* species_port.gen.h, GENERATED by tools/portspecies.py --header; DO NOT EDIT.",
        " *",
        " * Where a fill put the 156 species a Gen 5 cartridge adds, and which of the three",
        " * icon palettes each is drawn with.",
        " *",
        " * Every member is arithmetic, because each appended run has no gap in it: the",
        " * base plus the species less MMO_PORTED_FIRST. The palette is not arithmetic and",
        " * never will be, so it is a byte each, out of mmo/POKE_ICONS.",
        " *",
        " * The bases are the fill tool's own constants, emitted from it rather than",
        " * written again here: the place the bytes go and the place the client looks for",
        " * them are one number or they are a bug nobody sees until a box is opened. */",
        "",
