...
def render_header():
    ...
    out = [
        "/* openmmo_ability_ids.h, GENERATED by tools/portabilities.py --header; DO NOT EDIT.",
        " *",
        " * The abilities Gen 5 added, as this engine would have numbered them. Its own",
        " * generated/abilities.h stops at ABILITY_BAD_DREAMS (123), so its battle code",
        " * has no case for any of these and no name to write one with; a mod source that",
        " * gives one an effect brings its id from here.",
        " *",
        " * Read out of codegen/gen5/enums.json, which mmo/tools/gen5_tables.py writes from",
        " * a Black cartridge's own ability name table and holds to the decomp over the 123",
        " * both games name. The server's Ability enum and mmo/src/display_data.gen.h are",
        " * built from the same file, so an id means one thing in all three. */",
