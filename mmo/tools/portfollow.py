...
def fill_header(out: Path, hg: Path) -> int:
    ...
    lines = [
        "/* follower_fill.gen.h, GENERATED by tools/portfollow.py "
        "--fill-header; DO NOT EDIT.",
        " *",
        " * What a follower fill has to know that is NOT on the cartridge it",
        " * reads. A player has no decompilation checkout, so the three answers",
        " * this file carries are frozen here and the fill in the launcher",
        " * compiles them in; the bytes themselves still come from the",
        " * player's own Heart Gold or Soul Silver image and are never",
        " * shipped.",
        " *",
        " * kFollowerSrcMember  which mmodel.narc member of the SOURCE image",
        " *                     draws each follower, in the order a package",
        " *                     appends them, so member i lands at",
        " *                     MMO_FOLLOWFILL_DST_FIRST + i and wears",
        " *                     graphics id MMO_FOLLOWER_GFX_BASE + i.",
        " * kFollowerMode       the source game's own two-bit follow mode per",
        " *                     one of its map headers, plus bit 2 for the",
        " *                     eleven Bell Tower maps a Diglett is turned away",
        " *                     from. 255 is a header with no answer.",
        " * kFollowerMapSec     the map section each source header belongs to,",
        " *                     which is the talk's per-section condition",
        " *                     table. 255 is a header with no section.",
        " */",
        "",
