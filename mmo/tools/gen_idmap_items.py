...
def main():
    ...
    lines = [
        "/* idmap_items.gen.h, GENERATED by tools/gen_idmap_items.py; DO NOT EDIT.",
        " *",
        " * The item ids the OpenMMO server and the pokeplatinum engine both have.",
        " * Both number items by position in the same Gen-5 order (the server mints",
        " * 5000 + the index of codegen/items/item_names.json, the engine numbers by",
        " * generated/items.txt), so an id needs no translation, only this check that",
        " * the item is in both worlds. Ids absent here are an engine slot the Gen-5",
        " * table blanked out, an engine id with no item data, or a Gen-5 item past the",
        " * last one the engine draws; all of them MUST trap, see idmap.c.",
        " *",
        " * Each row's comment carries the third numbering: gbaID from the engine JSON,",
        " * which is the FireRed id or 0. Potion is engine 17 and gba 13; 13 is already",
        " * a Dusk Ball. Seating the gba number draws the wrong icon. %d of %d items"
        % (len(gba_ne), len(gba_set)),
        " * with a gbaID have a different platinum id; %d of those collide with another"
        % len(collisions),
        " * engine item.",
        " *",
        " * %d of %d shared ids carry the same name in both trees. The %d that do not"
        % (len(shared) - len(divergent), len(shared), len(divergent)),
        " * are marked below: a shared id holding different content, not a renumbering,",
        " * so the engine draws its own. */",
        "",
