...
def emit(out, species_max, egg, bad_egg, pokegra, otherpoke, forms, cases,
    ...
    w = out.write
    w("""/* sprite_index.gen.h, GENERATED by tools/gen_sprite_index.py; DO NOT EDIT.
 *
 * Which member of which archive holds a Pokemon's picture, read out of the
 * pokeplatinum engine's own BuildPokemonSpriteTemplate and bounded by the
 * archives the engine's ROM build actually produced.
 *
 * Two archives. pl_pokegra holds every species with no alternate form, %d
 * members to the species in the order (back female, back male, front female,
 * front male, normal palette, shiny palette), so the last species it can
 * answer for is %d and there is nothing after it. pl_otherpoke holds the
 * multi-form ones, at %d members with no per-species stride at all: each of the
 * %d species below has its own base and its own way of counting.
 *
 * MMO_SPRITE_FORMS is the engine's own form-count clamp, from
 * Pokemon_SanitizeFormId. A species that is not in it has exactly one form. */
...
""" % (POKEGRA_STRIDE, species_max, otherpoke, len(cases), species_max, egg,
       bad_egg, POKEGRA_STRIDE, pokegra, otherpoke, genders["GENDER_FEMALE"],
       faces["FACE_BACK"], faces["FACE_FRONT"], spinda))
