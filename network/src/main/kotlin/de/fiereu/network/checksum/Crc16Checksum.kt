...
class Crc16Checksum : Checksum {
  ...
  override fun verify(data: ByteBuf, expected: ByteArray): Boolean {
    ...
    val actual = hash(data)
    val expectedSum = ((expected[1].toInt() and 0xFF) shl 8) or (expected[0].toInt() and 0xFF)
    return actual == expectedSum
  }

  private fun hash(data: ByteBuf): Int {
    var sum = 0
    val end = data.writerIndex()
    var i = data.readerIndex()
    while (i < end) {
      val b = data.getByte(i).toInt() and 0xFF
      sum = (sum ushr 8) xor TABLE[(sum xor b) and 0xFF]
      i++
    }
