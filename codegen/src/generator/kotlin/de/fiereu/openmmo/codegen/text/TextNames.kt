...
/**
 * What a DS text bank and its messages are called in generated Kotlin. Shared, because a script
 * that shows a line has to name the same enum entry the text generator emitted for it, and two
 * copies of this rule would drift into a port that does not compile.
 */
object TextNames {
  ...
  /**
   * A message's label already carries its bank, `TwinleafTown_Text_BigThud` in `twinleaf_town`, so
   * the enum entry is what is left after that prefix.
   */
